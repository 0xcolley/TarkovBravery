...
def generate_sql_insert(json_file_path):
    ...
    for item in items:
        ...
        if "carrier" not in name.lower():  # Exclude items containing "carrier"
            image = item['image512pxLink']
            sql_insert_statements.append(f"INSERT INTO Armor (name, image) VALUES ('{name}', '{image}');")
...
def main():
    json_file_path = './item_jsons/armor.json'
    sql_file_path = './out/armor.sql'
